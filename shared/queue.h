 ...
 * ptr_elem  : pointeur vers l'élément à chainer (do tipo especificado por type)
 * head      : pointeur vers la tête de liste (do tipo link)
 ...
 */
static __inline__ int list_is_empty(link *head)
{
...
static __inline__ void *__queue_out(link *head, unsigned long diff)
{
	...
	if (list_is_empty(head)) return ((void *)0);
